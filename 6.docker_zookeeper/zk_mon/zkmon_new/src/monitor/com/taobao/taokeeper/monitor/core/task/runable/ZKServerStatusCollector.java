...
public class ZKServerStatusCollector implements Runnable {
	...
	private void sshZooKeeperAndHandleStat(String ip, int port, ZooKeeperStatus zooKeeperStatus) {
		...
		try {
			sshResource = SSHUtil.executeWithoutHandleBufferedReader(ip, SystemConstant.portOfSSH, userNameOfSSH, passwordOfSSH, StringUtil.replaceSequenced(COMMAND_STAT, ip, port + EMPTY_STRING));
			if (null == sshResource) {
	...
	private void telnetZooKeeperAndHandleWchs(String ip, int port, ZooKeeperStatus zooKeeperStatus) {
		...
		try {
			...
			String wchsOutput = SSHUtil.execute(ip, SystemConstant.portOfSSH, userNameOfSSH, passwordOfSSH, StringUtil.replaceSequenced(COMMAND_WCHS, ip, port + EMPTY_STRING));
	...
	private void sshZooKeeperAndHandleWchc(String ip, int port, ZooKeeperStatus zooKeeperStatus, int clusterId) {
		...
		try {
			...
			String wchcOutput = SSHUtil.execute(ip, SystemConstant.portOfSSH, userNameOfSSH, passwordOfSSH, StringUtil.replaceSequenced(COMMAND_WCHC, ip, port + EMPTY_STRING));
	...
	private void sshZooKeeperAndHandleRwps(String ip, int port, ZooKeeperStatusV2 zooKeeperStatus, int clusterId) {
		...
		try {
			...
			String rwpsOutput = SSHUtil.execute(ip, SystemConstant.portOfSSH, userNameOfSSH, passwordOfSSH, StringUtil.replaceSequenced(COMMAND_RWPS, ip, port + EMPTY_STRING));
	...
	private void storeTaoKeeperStatToDB(int clusterId, ZooKeeperStatusV2 zooKeeperStatus) {
		...
		try {
			...
			reportDAO.addTaoKeeperStat(new TaoKeeperStat(clusterId, zooKeeperStatus.getIp(), DateUtil.getNowTime(DateFormat.DateTime), DateUtil.getNowTime(DateFormat.Date), MapUtil.size(zooKeeperStatus.getConnections()), zooKeeperStatus.getWatches(), Long.parseLong(zooKeeperStatus.getSent() == null ? "0" : zooKeeperStatus.getSent()), Long.parseLong(zooKeeperStatus.getReceived() == null ? "0" : zooKeeperStatus.getReceived()), zooKeeperStatus.getNodeCount(), rwStatistics));
		} catch (NumberFormatException e) {
